...
from tools.router import plan_tool_from_message
...
def _wants_package_json_scripts(user_message: str) -> bool:
    lower = user_message.lower()
    return "package.json" in lower and ("script" in lower or "scripts" in lower or "package" in lower)
...
- Never claim an approval request exists in the final answer. Only the write/shell tool execution can create a real approval request.
- For terminal/current directory/process/package checks, choose run_shell_command.
- Preserve exact requested shell commands. If user says `dir`, use `dir`, not `ls`.
...
""".strip()
    ...
    try:
        ...
        # If the model wants to answer directly but the user clearly asked about
        # memory, force a one-time memory tool call.
        if (
            fallback
            and fallback.tool_name == "search_memory"
            and normalized.action == "direct_answer"
        ):
